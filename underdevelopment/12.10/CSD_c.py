...
#             plt.savefig(filename + "/iteration_conv_{bt}.png".format(bt=bt_param))
    ...
    # TODO: output the solution trajectory, obj_val history, 
...
def grad(sol, 
         num_drv, num_task, num_node, num_depot, 
         Prob_rs, Prob_sd, Prob_rs_1):
...
def obj(sol, 
        num_drv, num_task, logit_param_q, logit_param_n,
        sumexp_od, sumexp_rs_k):
...
def allocation_under_price(price, exp_cost_or, cost_rs, exp_cost_sd, num_drv, num_task, num_node, num_depot, logit_param_q, logit_param_n):
    ...
    real_allocation_n = np.hstack((real_allocation_n, num_task - np.sum(real_allocation_n, axis=1).reshape([-1,1])))
...
def roundoff_allocation(allocation_q, allocation_n, num_driver, num_task):
    ...
    f_q = allocation_q
    f_n = np.delete(allocation_n, -1, axis=1)
    ...
    round_f_n = np.hstack((round_f_n, num_task - np.sum(round_f_n, axis=1).reshape([-1,1])))
...
def generate_demand(num_node, num_depot, num_sum_driver):
    ...
    nn = np.random.rand(num_node*num_depot)
    ...
    #縦の行列にして返す
    return q.reshape([-1,1]), n.reshape([-1,1])
...
def create_costmat(num_node, num_depot, distmat, depot_list):
    ...
    print("completed making all-nodes costs")
    return costmat
...
class DataMaker:
    ...
    def save_demand_supply(self, path):
        np.savetxt(path, np.vstack((self.num_drv, self.num_task)), delimiter = ",")
...
#testdata = DataMaker(16, 3, 100, 5, 5)


# In[305]:


#testdata.make_demand_supply()
#print(testdata.num_drv, testdata.num_task)


# In[306]:


#testdata.make_network()
#nx.draw(testdata.network)


# In[307]:


#testdata.make_distmat()
#print(testdata.distmat)


# In[308]:


#testdata.make_depot_list()
#print(testdata.depot_list)


# In[309]:


#testdata.make_costmat()
#print(testdata.costmat)


# In[310]:


#testdata.make_atomic_costmat()
#print(testdata.atomic_costmat_q, testdata.atomic_costmat_n)


# In[311]:


#test_csd = testdata.create_CSD()
#print(type(test_csd))
...
#print(test_csd.opt_price,
        #test_csd.opt_welfare,
        #test_csd.time_for_solve)


# In[317]:


#test_csd.allocate_tasks()
#print(test_csd.task_allocation_q, test_csd.task_allocation_n)