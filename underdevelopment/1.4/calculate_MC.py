...
def compute_mu_sd_and_P_sd(Z_sd, mu_sd, P_sd_rs, t, node_to_index, d_nodes, r_nodes, s_nodes, theta_p, v):
    ...
    for n in range(1, iterations):
        ...
        for s_idx, s_node in enumerate(s_nodes): #s_idx is the index of s starting from 0, used for Z matrix for example
            ...
            for d_idx in range(d_size): #d index is the same anywhere
                sum_tot = 0.0
                ...
                Z_val = exp(-theta_p*t[s_index,d_idx]) + sum_tot
...
def compute_d_mu_od(d_mu_od, P_od_rs, d_mu_sd, n_size, r_nodes, o_nodes, d_nodes, s_nodes):
...
def compute_MC_and_grad_MC(o_dict, r_dict, s_dict, d_dict, n_size, K, theta_p, theta_c, z_rs_bar, y_od_bar, c_rs, seed, v):
    ...
    Z_od, mu_od, P_od_rs,d_mu_od = initialize_upstream_arrays(o_size, d_size, r_size, s_size)
    
    compute_mu_od_P_od(Z_od, mu_od, P_od_rs, t, node_to_index, d_nodes, o_nodes, r_nodes, s_nodes, mu_sd, n_size, theta_p, v)
    
    compute_d_mu_od(d_mu_od, P_od_rs, d_mu_sd, n_size, r_nodes, o_nodes, d_nodes, s_nodes)
...
def compute_y_distrib(r_size,n_size,s_size, o_size, d_size,y_od_bar,P_od_rs,P_sd_rs,node_to_index,d_nodes,o_nodes,r_nodes,s_nodes):
