...
def main():
    ...
    """
    origins = [f"O{i}" for i in range(1,11)]
    r_nodes = [f"R{i}" for i in range(1,51)]
    s_nodes = [f"S{i}" for i in range(1,51)]
    destinations = [f"D{i}" for i in range(1,11)]
    ...
    fista = FISTA(subalgorithm, L0=1, eta=5, epsilon=1e-3, max_iter=500)
